  ...
  <ArtistLayout>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-semibold text-white">
          Detalle composición · {{ composition.title }}
        </h1>
        <p class="text-gray-400 text-sm">
          Periodo: {{ statement.reporting_period || "-" }}
        </p>
      </div>
      <Link :href="route('artist.compositions.royalties.index', composition.id)" class="text-gray-400 hover:text-white">
        Volver
      </Link>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
      <div class="bg-[#0f0f0f] border border-[#2a2a2a] rounded-lg p-4">
        <h3 class="text-[#ffa236] font-semibold mb-2">Total composición</h3>
        <div class="text-sm text-gray-300 space-y-2">
          <div><span class="text-gray-500">Units:</span> {{ summary.units_total }}</div>
          <div><span class="text-gray-500">Total USD:</span> {{ formatMoney(summary.total_composition_usd) }}</div>
        </div>
      </div>
      ...
      <table class="min-w-full text-sm text-gray-300">
        <thead class="bg-[#1c1c1c] text-gray-400 uppercase text-xs">
          <tr>
            ...
            <th class="px-4 py-2 text-left">Total línea USD</th>
            <th class="px-4 py-2 text-left">Mi línea USD</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            ...
            <td class="px-4 py-3">{{ formatMoney(line.gross_line_usd) }}</td>
            <td class="px-4 py-3">{{ formatMoney(line.my_share_usd) }}</td>
          </tr>
