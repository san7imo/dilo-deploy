...
const sharePoolOptions = [
  { value: "writer", label: "Pool Writers" },
  { value: "publisher", label: "Pool Publishers" },
  { value: "mechanical_payee", label: "Pool Mechanical Payee" },
];
...
const form = useForm({
  ...
  participants: [
    ...
    createParticipant({
      share_pool: "publisher",
      role: "publisher",
      participant_type: "manual",
      percentage: 100,
    }),
    createParticipant({
      share_pool: "mechanical_payee",
      role: "publisher",
      participant_type: "manual",
      percentage: 100,
    }),
...
const poolTotals = computed(() => {
  ...
  form.participants.forEach((participant) => {
    const pool = participant.share_pool || "writer";
    if (!Object.prototype.hasOwnProperty.call(totals, pool)) return;
    totals[pool] += Number(participant.percentage || 0);
  });

  return totals;
});

const poolBadgeClass = (pool) => {
  if (pool === "writer") return "border-emerald-500/40 bg-emerald-500/10 text-emerald-200";
  if (pool === "publisher") return "border-sky-500/40 bg-sky-500/10 text-sky-200";
  return "border-amber-500/40 bg-amber-500/10 text-amber-200";
};

const poolTotalClass = (pool) => {
  const total = Number(poolTotals.value[pool] || 0);
  if (Math.abs(total - 100) <= 0.01) return "text-emerald-300";
  return "text-red-300";
};
...
<template>
  <AdminLayout title="Crear split de composición">
    <div class="flex items-center justify-between mb-6">
      ...
      <BackNavButton
        :href="route('admin.compositions.splits.index', { composition: composition.id, back: back_url || undefined })"
      />
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div class="panel">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mb-4">
          <div
            v-for="pool in sharePoolOptions"
            :key="pool.value"
            class="rounded-lg border px-3 py-2 text-sm"
            :class="poolBadgeClass(pool.value)"
          >
            <p class="text-[11px] uppercase tracking-wide opacity-90">{{ pool.label }}</p>
            <p class="font-semibold" :class="poolTotalClass(pool.value)">
              {{ Number(poolTotals[pool.value] || 0).toFixed(2) }}%
            </p>
          </div>
        </div>
          ...
          >
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
              ...
              <div class="lg:col-span-2">
                ...
                <select v-model="participant.share_pool" class="input">
                  <option
                    v-for="option in sharePoolOptions"
                    :key="option.value"
                    :value="option.value"
                  >
                    {{ option.label }}
              ...
              <div class="lg:col-span-2">
                ...
                <p class="text-[11px] text-gray-500 mt-1">
                  {{ sharePoolLabel(participant.share_pool) }}
                </p>
              </div>
            </div>
            ...
            <p v-if="form.errors[`participants.${index}.share_pool`]" class="text-red-500 text-xs">
              {{ form.errors[`participants.${index}.share_pool`] }}
            </p>
