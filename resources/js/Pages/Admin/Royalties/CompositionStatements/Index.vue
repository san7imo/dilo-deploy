...
const handleProcess = (id) => {
  router.post(route("admin.royalties.composition-statements.process", id));
};
</script>

<template>
  <AdminLayout title="Composition Royalties">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="text-2xl font-semibold text-white">Composition Royalties · Statements</h1>
      ...
      <div class="flex items-center gap-2">
        ...
        <a :href="route('admin.royalties.composition-statements.template')" class="btn-secondary">
          Descargar plantilla CSV
        </a>
        <Link :href="route('admin.royalties.composition-statements.create')" class="btn-primary">
          Upload composición
        </Link>
      </div>
    </div>
      ...
      <table class="min-w-full text-sm text-gray-300">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-3">{{ statement.provider }}</td>
            <td class="px-4 py-3">{{ statement.source_name || "-" }}</td>
            <td class="px-4 py-3">{{ statement.reporting_period || "-" }}</td>
            <td class="px-4 py-3">v{{ statement.version ?? 1 }}</td>
            <td class="px-4 py-3">
              <span class="px-2 py-1 rounded bg-[#2a2a2a] text-xs uppercase">
                {{ statement.status }}
              </span>
            </td>
            <td class="px-4 py-3">{{ statement.total_lines ?? 0 }}</td>
            <td class="px-4 py-3">{{ formatMoney(statement.total_amount_usd) }}</td>
            <td class="px-4 py-3">{{ statement.creator?.name || "-" }}</td>
            <td class="px-4 py-3 text-right">
              <RowActionMenu label="Acciones statement composición">
                <Link
                  :href="route('admin.royalties.composition-statements.show', statement.id)"
                  class="block rounded px-3 py-2 text-sm text-[#ffa236] hover:bg-white/10"
                >
                  Ver
                </Link>
                <a
                  :href="route('admin.royalties.composition-statements.download', statement.id)"
          ...
          <tr v-if="!statements.data?.length">
            <td colspan="10" class="px-4 py-6 text-center text-gray-400">
              No hay statements de composición cargados.
            </td>
          </tr>
        </tbody>
      </table>
