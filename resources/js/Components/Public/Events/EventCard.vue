    ...
    <div class="p-4">
      ...
      <div class="space-y-1 mt-2">
        <p class="text-zinc-400 text-sm truncate">
          ...
          <span v-else>{{ event.location || 'Ubicación no disponible' }}</span>
        </p>
        <p v-if="event.venue_address" class="text-zinc-500 text-xs truncate">{{ event.venue_address }}</p>
      </div>
      ...
      <div v-if="event.artists && event.artists.length > 0" class="mt-3 pt-3 border-t border-white/10">
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="(artist, idx) in event.artists.slice(0, 2)" :key="idx"
            class="text-xs bg-zinc-800 text-white px-2 py-1 rounded">
            {{ artist.name }}
          </span>
          <span v-if="event.artists.length > 2" class="text-xs bg-zinc-800 text-zinc-300 px-2 py-1 rounded">
